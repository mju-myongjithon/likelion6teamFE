...
import { PlaceVoteCard, type PlaceVoteOption } from "../components/ds/chat/PlaceVoteCard";
...
const TONES: Record<string, AvatarTone> = { "김민준": "violet", "이서연": "pink", "박도윤": "emerald", "최유진": "orange", "강민서": "violet", "나": "violet", "정하늘": "emerald", "윤재원": "pink" };

/** 약속 잡기 · 장소 투표 — 카카오톡 스타일 투표 카드로 장소를 확정한다. */
export function MeetupVotePage(): JSX.Element {
  const navigate = useNavigate();
  const [picked, setPicked] = React.useState<string>("gangnam");
  const [options, setOptions] = React.useState<PlaceVoteOption[]>(INITIAL);
      ...
      <div style={{ display: "flex", height: "100%", minHeight: 640 }}>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, background: "var(--surface-soft)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "var(--space-md) var(--space-lg)", borderBottom: "1px solid var(--hairline)", background: "var(--canvas)" }}>
            <a onClick={(e) => { e.preventDefault(); navigate("/chat"); }} href="/chat" style={{ display: "inline-flex", color: "var(--muted)" }}><Icon name="arrow-left" size={18} color="var(--muted)" /></a>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 20, display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <ChatMessage mine>
              <PlaceVoteCard title="2/1(토) 스터디 장소" options={options} selectedId={picked} deadline="오늘 오후 6시 마감" onVote={vote} />
            </ChatMessage>
