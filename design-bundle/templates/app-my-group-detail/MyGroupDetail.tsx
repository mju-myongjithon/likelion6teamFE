/**
 * MyGroupDetail — CampusLink 내 모임 상세 + 탈퇴. Typed handoff source.
 */
...
function LeaveModal({ onClose }: { onClose: () => void }): JSX.Element {
  const { Button, Icon } = getDS<DS>();
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 24 }}>
      <div style={{ background: "var(--canvas)", borderRadius: "var(--radius-xl)", padding: 28, width: "100%", maxWidth: 380, boxShadow: "var(--shadow-raised)" }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <Button variant="primary" size="lg" fullWidth onClick={onClose} style={{ background: "var(--error)" }}>탈퇴하기</Button>
        </div>
      </div>
    </div>
  );
...
export function MyGroupDetail(): JSX.Element {
  const { Button, Badge, Avatar, Icon } = getDS<DS>();
  const [leaving, setLeaving] = React.useState<boolean>(false);
  return (
    <AppShell active="my">
      <div style={{ padding: 28, maxWidth: 780 }}>
        <a href="../app-my-groups/MyGroups.dc.html" style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "var(--muted)", fontFamily: "var(--font-sans)", fontSize: 14, fontWeight: 500, textDecoration: "none", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 20 }}>
          ...
          <Button variant="primary" iconLeft={<Icon name="message-circle" size={16} color="var(--on-primary)" />} onClick={() => { window.location.href = "../app-chat/Chat.dc.html"; }}>채팅방</Button>
        </div>
