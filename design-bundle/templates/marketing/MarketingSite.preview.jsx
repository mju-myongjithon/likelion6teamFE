/**
 * MarketingSite.preview.jsx — RUNTIME MOUNT for the template preview only.
 *
 * This is the plain-JS build that the design-system template runtime mounts
 * (the runtime resolves neither ES imports nor TypeScript syntax, so it needs
 * global React + a window-namespace lookup). The canonical, typed, handoff
 * source is MarketingSite.tsx in this folder — edit there and mirror here.
 */
...
function Section({ children, bg = "var(--canvas)", pad = 96, style = {} }) {
  return (
...
// ── Template-runtime mount ──────────────────────────────────────────────────
// The DC runtime renders x-import React components with its OWN React instance,
// which collides with the helmet's UMD React that this file + the _ds_bundle.js
// use (dual-React → null hook dispatcher). To keep a single React instance for
// render, hooks, and the bundle, we mount our OWN React root inside a custom
// element and expose that element (not a React component) to the runtime.
class CampusLinkMarketingElement extends HTMLElement {
  connectedCallback() {
    const boot = () => {
      if (!window.React || !window.ReactDOM || typeof window[DS_NAMESPACE] !== "object") {
        setTimeout(boot, 40);
        return;
      }
      React = window.React;
      if (!this._root) this._root = window.ReactDOM.createRoot(this);
      this._root.render(React.createElement(MarketingSite));
    };
    boot();
  }
...
if (!customElements.get("campuslink-marketing")) {
  customElements.define("campuslink-marketing", CampusLinkMarketingElement);
}
